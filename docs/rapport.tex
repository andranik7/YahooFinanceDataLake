...
\midrule
Orchestration & Apache Airflow 2.7.3 & Planification et exécution des tâches \\
Transformation & Apache Spark 3.5.0 & Traitement distribué des données \\
Indexation & Elasticsearch 8.11.0 & Moteur de recherche et analytics \\
Visualisation & Kibana 8.11.0 & Dashboards interactifs \\
Base métadonnées & PostgreSQL 15 & Backend Airflow \\
Runtime Java & OpenJDK 11 & Requis pour Spark dans Airflow \\
...
├── airflow/
│   └── dags/
│       └── yahoo_finance_pipeline.py  # DAG principal
├── config/
...
\begin{itemize}
    \item \textbf{Stocks} : historique 12 mois (Open, High, Low, Close, Volume)
...
\begin{itemize}
    ...
    \item Python 3.10+ (optionnel, pour développement local)
    \item Poetry (optionnel, gestionnaire de dépendances)
\end{itemize}
...
# Construire l'image Airflow (avec Java pour Spark)
...
\textbf{Note :} L'image Airflow custom (\texttt{Dockerfile.airflow}) inclut Java 11 et PySpark pour permettre la soumission de jobs Spark via \texttt{SparkSubmitOperator}.
...
\subsubsection{DAG : yahoo\_finance\_pipeline}

\begin{figure}[H]
\centering
\fbox{\parbox{0.9\textwidth}{
\centering
\texttt{start} $\rightarrow$ \texttt{[ingest\_stocks, ingest\_news]} $\rightarrow$ \texttt{format\_data} $\rightarrow$ \texttt{combine\_data} $\rightarrow$ \texttt{index\_data} $\rightarrow$ \texttt{end}
}}
\caption{Flux du DAG Airflow}
\end{figure}
...
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Task} & \textbf{Opérateur} & \textbf{Description} \\
\midrule
ingest\_stocks & PythonOperator & Collecte cours + infos entreprises \\
ingest\_news & PythonOperator & Collecte actualités financières \\
format\_data & SparkSubmitOperator & Conversion JSON → Parquet \\
combine\_data & SparkSubmitOperator & Jointure + métriques dérivées \\
index\_data & PythonOperator & Indexation Elasticsearch \\
\bottomrule
\end{tabular}
\caption{Tasks du DAG}
\end{table}

\begin{lstlisting}[language=Python, caption=Configuration SparkSubmitOperator]
...
\textbf{Exécution :}
\begin{enumerate}
    \item Accéder à Airflow : \url{http://localhost:8080} (admin/admin)
    \item Activer le DAG \texttt{yahoo\_finance\_pipeline}
    \item Déclencher manuellement avec "Trigger DAG" ou attendre l'exécution planifiée (@daily)
\end{enumerate}

\textbf{Indexation incrémentale :} Le pipeline ajoute les nouvelles données sans supprimer l'historique existant. Les documents sont mis à jour via leur identifiant unique (\texttt{symbol\_date} pour les stocks, \texttt{uuid} pour les news).

\subsection{Exécution manuelle (optionnel)}
...
\midrule
Stock prices & 1 280 & 12 mois & Journalière \\
Company info & 10 & - & Par symbole \\
News & 83 & Variable & Par article \\
Enriched stocks & 1 280 & 12 mois & Journalière \\
\bottomrule
\end{tabular}
...
\begin{itemize}
    \item Intégration de sources de données supplémentaires (Reuters, Bloomberg)
    \item Alertes sur les variations de cours importantes
    \item Machine Learning pour la prédiction de tendances
    \item Analyse de sentiment sur les news
    \item Scaling horizontal du cluster Spark
\end{itemize}
...
\begin{itemize}
    ...
    \item Orchestration automatisée via DAG Airflow avec SparkSubmitOperator
